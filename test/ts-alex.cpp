...
private:
  void onInterest(const ndn::InterestFilter& filter, const ndn::Interest& interest) {
    ...
    if (cmd.compare("COMMIT") == 0) {
      consume("ndn/thu/paxos/node1/PREPARE");
    }
  }
...
void
printPeriod(ndn::Scheduler* scheduler)
{
  std::cerr << ".";
  scheduler->scheduleEvent(ndn::time::seconds(1),
                           std::bind(printPeriod, scheduler));
}

int main(int argc, char** argv) {

  if (argc < 2) {
    std::cerr << "Usage: node0 or node1" << std::endl;
    return 0;
  }
  ...
  ndn::Scheduler scheduler(ts.face_->getIoService());
  printPeriod(&scheduler);
